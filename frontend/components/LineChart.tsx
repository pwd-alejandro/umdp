...
export default function DepartamentoLineChart() {
  ...
  <div className="flex gap-6 p-6">
    ...
    <div className="flex-1">
      <h2 className="text-xl font-semibold mb-4">Promedios por departamento</h2>
      <ResponsiveContainer width="100%" height={500}>
        <LineChart data={data}>
          <XAxis dataKey="test_year" />
          <YAxis />
          <Tooltip />
          <Legend />
          {(selectedDepartamentos.length ? selectedDepartamentos : departamentos).map(
            (depto) => (
              <Line
                key={depto}
                type="monotone"
